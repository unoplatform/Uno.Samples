...
public partial record SocialFeedModel
{
    ...
    {
        Activity("Priya Sharma", MovieData.PortraitWoman, "is watching", "The Last Horizon", "2m ago",
            "Mind-blowing cinematography! Can't believe the ending.", 14, true),
        Activity("Marcus Chen", MovieData.PortraitMan, "just finished", "Crimson Protocol", "18m ago",
            "Action-packed from start to finish. 9/10 would rewatch.", 31, true),
        Activity("Sofia Reyes", MovieData.PortraitYoung, "added to watchlist", "Between Worlds", "1h ago",
            "", 8, false),
        Activity("Jordan Kato", MovieData.PortraitProfile, "rated 5 stars", "Earth Reborn", "2h ago",
            "Everyone needs to watch this. Absolutely stunning.", 47, false),
        Activity("Priya Sharma", MovieData.PortraitWoman, "started watching", "Void Walker", "3h ago",
            "Okay this is actually terrifying... watching with lights on.", 22, true),
        Activity("Alex Müller", MovieData.PortraitFriends, "just finished", "Solar Drift", "5h ago",
            "Better than The Last Horizon. The score alone is worth it.", 19, false),
        Activity("Marcus Chen", MovieData.PortraitMan, "added to watchlist", "Iron Veil", "Yesterday",
            "", 5, true),
    };

    public IReadOnlyList<FriendSuggestion> SuggestedFriends => new[]
    {
        new FriendSuggestion("Nadia Okonkwo", MovieData.PortraitWoman, "12 movies in common"),
        new FriendSuggestion("Luca Ferretti", MovieData.PortraitMan, "8 movies in common"),
        new FriendSuggestion("Yuna Park", MovieData.PortraitYoung, "Sci-Fi enthusiast"),
    };

    public IReadOnlyList<Movie> TrendingAmongFriends => MovieData.Ids("m-001", "m-009", "m-002");

    private static FriendActivity Activity(
        string name, string avatar, string action, string title, string timeAgo,
        string comment, int likes, bool online) =>
        new(name, avatar, action, MovieData.ByTitle(title), timeAgo, comment, likes, online);
}
