...
## Skills & lessons — use these first

Before any Uno UI/XAML work on a sample in this repo, consult **both**:

- The canonical **`uno-*` Uno Platform skills** (responsive, navigation, lightweight styling,
  semantic theme brushes, ancestor binding, UI testing, …). They are the source of truth for
  the Uno APIs and ship compilable templates. They live in the `unoplatform/studio` repo and
  are published as the `uno-platform-studio` Claude Code plugin.
- The **`uno-sample-lessons` skill** (and its backing `specs/lessons.md`) for the
  repo-specific gotchas the general skills don't cover. When a lesson cites a `Skill:`, read
