...
namespace Authentication.MsalExtensionsDemo.Services;

/// <summary>Outcome of a Microsoft Graph call.</summary>
public sealed record GraphResult(bool IsSuccess, int StatusCode, string Body, string? DisplayName, string? UserPrincipalName, string? Mail, string? JobTitle);

/// <summary>
/// Calls Microsoft Graph with the access token the authentication stack produced, to prove the
/// token actually works against a real API.
/// </summary>
/// <remarks>
/// Deliberately a plain <see cref="HttpClient"/> rather than the Graph SDK: it shows the one
/// thing that matters - putting the access token in an <c>Authorization: Bearer</c> header. In a
/// real Uno.Extensions app you would instead register a handler-decorated HttpClient via
/// <c>UseHttp</c> so the token is attached automatically.
/// </remarks>
