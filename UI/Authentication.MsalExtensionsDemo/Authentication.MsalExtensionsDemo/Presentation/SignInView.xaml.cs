    ...
    private void OnDataContextChanged(FrameworkElement sender, DataContextChangedEventArgs args)
    {
        ...
        if (viewModel.Log.Count == 0)
        {
            viewModel.Log.Info(
                ...
                Press Sign in to run the flow a production app should use: the provider checks
                the token cache first, and only shows UI when MSAL reports that interaction is
                required.
                """);
        }
