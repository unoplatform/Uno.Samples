...
{
    public class MountainDataItem : INotifyDataErrorInfo, IComparable
    {
        ...
        {
            int lnCompare = Range.CompareTo((obj as MountainDataItem).Range);

            if (lnCompare == 0)
            {
                return Parent_mountain.CompareTo((obj as MountainDataItem).Parent_mountain);
            }
            else
            {
                return lnCompare;
